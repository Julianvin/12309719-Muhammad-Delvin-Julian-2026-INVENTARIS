...
<div class="bg-white rounded hover:shadow-sm transition-shadow p-8 w-full border border-gray-50 h-[calc(100vh-120px)] overflow-y-auto">
    <!-- Header -->
    <div class="mb-10">
        <h2 class="text-[#03346E] text-[22px] font-bold mb-2">Add Item Forms</h2>
        <p class="text-[#7A8599] text-[15px]">Please <span class="text-[#D66D92]">.fill-all</span> input form with right value.</p>
    </div>

    <!-- Form -->
    <form action="{{ url('/admin/items') }}" method="POST" class="space-y-8 max-w-5xl">
        @csrf
        ...
        <div>
            ...
            <select id="category_id" name="category_id" 
                    class="w-full border @error('category_id') border-red-500 @else border-[#F1F3F5] @enderror hover:border-gray-300 rounded-sm px-6 py-4 text-[15px] text-[#A0AABF] outline-none focus:border-blue-400 transition-colors bg-white">
        ...
        <div>
            ...
            <div class="flex border @error('total') border-red-500 @else border-[#F1F3F5] @enderror hover:border-gray-300 focus-within:border-blue-400 rounded-sm overflow-hidden transition-colors">
                <!-- Input -->
                <input type="number" id="total" name="total" value="{{ old('total') }}" placeholder="10" 
