...
<div class="bg-white rounded hover:shadow-sm transition-shadow p-8 w-full border border-gray-50 h-[calc(100vh-120px)] overflow-y-auto">
    <!-- Header -->
    <div class="mb-10">
        <h2 class="text-[#03346E] text-[22px] font-bold mb-2">Edit Account Forms</h2>
        <p class="text-[#7A8599] text-[15px]">Please <span class="text-[#D66D92]">.fill-all</span> input form with right value.</p>
    </div>

    <!-- Form -->
    <form action="{{ url('/admin/users/' . $user->id) }}" method="POST" class="space-y-8 max-w-5xl">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <div class="relative">
                <input type="text" id="name" name="name" value="{{ old('name', $user->name) }}" placeholder="Nama Akun" 
        ...
        <div>
            ...
            <div class="relative">
                <input type="email" id="email" name="email" value="{{ old('email', $user->email) }}" placeholder="Email Akun" 
        ...
        <!-- New Password Field -->
        <div>
            <label for="password" class="block text-[#03346E] text-[17px] mb-3">New Password <span class="text-[#EAB308] text-sm italic">optional</span></label>
            <div class="relative">
                <input type="password" id="password" name="password" placeholder="Masukkan password baru jika ingin mengganti" 
                       class="w-full border @error('password') border-red-500 @else border-[#F1F3F5] @enderror hover:border-gray-300 rounded-sm px-6 py-4 text-[15px] text-gray-800 outline-none focus:border-blue-400 placeholder-[#D1D5DB] transition-colors" />
                @error('password')
                <div class="absolute inset-y-0 right-0 pr-4 flex items-center pointer-events-none">
                    <svg class="h-5 w-5 text-red-500" fill="currentColor" viewBox="0 0 20 20">
                        <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd" />
        ...
        <div class="flex justify-end space-x-4 pt-10">
            <a href="{{ url('/admin/users/' . $user->role) }}" class="bg-[#A1A1AA] hover:bg-gray-500 text-gray-900 px-10 py-3.5 rounded text-[15px] font-medium transition shadow-sm">
                Cancel
            </a>
