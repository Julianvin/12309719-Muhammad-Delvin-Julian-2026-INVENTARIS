...
<div class="bg-white rounded hover:shadow-sm transition-shadow p-8 w-full border border-gray-50 h-full">
    <!-- Header -->
    <div class="mb-8">
        <h2 class="text-[#03346E] text-[22px] font-bold mb-2">Edit Category Forms</h2>
        <p class="text-[#7A8599] text-[15px]">Please modify properties of <span class="text-[#D66D92]">{{ $category->name }}</span>.</p>
    </div>

    <!-- Form -->
    <form action="{{ url('/admin/categories/' . $category->id) }}" method="POST" class="space-y-8 max-w-4xl">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <div class="relative">
                <input type="text" id="name" name="name" value="{{ old('name', $category->name) }}" placeholder="Alat Dapur" 
            ...
            <div class="relative">
                <div class="flex border @error('division') border-red-500 @else border-[#A6C4E1] @enderror rounded-sm bg-white overflow-hidden focus-within:border-blue-400 transition-colors">
                    ...
                    <select id="division" name="division" class="flex-1 px-5 py-4 text-[15px] text-[#334155] outline-none">
                        ...
                        <option value="Sarpras" {{ old('division', $category->division_pj) == 'Sarpras' ? 'selected' : '' }}>Sarpras</option>
                        <option value="Tata Usaha" {{ old('division', $category->division_pj) == 'Tata Usaha' ? 'selected' : '' }}>Tata Usaha</option>
                        <option value="tefa" {{ old('division', $category->division_pj) == 'tefa' ? 'selected' : '' }}>tefa</option>
                    </select>
                </div>
                @error('division')
                <div class="absolute inset-y-0 right-0 pr-4 flex items-center pointer-events-none">
